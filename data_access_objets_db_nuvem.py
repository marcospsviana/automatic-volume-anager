...
class DataAccessObjectsNuvem(object):
    def __init__(self):
        ...
        self.__conn = mdb.connect(host = self.__host, user = self.__user, password = self.__passwd, database = self.__database)
    ...
    def create_user(self, id_user, nome, email, telefone):
        ...
        self.__cursor.execute("INSERT INTO tb_usuario (id_usuario, nome, email, telefone) values ('%s','%s','%s','%s')" % ( __id_user, __nome, __email,  __telefone))
        self.__conn.commit()
    ...
    def cadastrar_armario(self, id_armario, classe, terminal, coluna, nivel, porta, compartimento):
        ...
        if select_porta == None or select_porta == [] or select_porta == "":
            ...
            self.__conn.commit()
            self.__conn.close()
            DAON.cadastrar_armario(id_armario, classe, terminal, coluna, nivel, porta, compartimento)
