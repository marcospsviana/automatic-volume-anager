...
class DataAccessObjectsManager(object):
    def __init__(self):
        ...
        self.__conn = mdb.connect(
            host=self.data_dao.db_host(),
            user=self.data_dao.db_user(),
            password=self.data_dao.db_passwd(),
            database=self.data_dao.db_database(),
        )
    ...
    def create_user(self, nome, email, telefone):
        ...
        if self.select.empty:
            ...
            from coolbagsafe_system.data_access_objets_db_nuvem import (
                DataAccessObjectsNuvem as DAON,
            )
            ...
            self.__c.execute(
                "UPDATE tb_usuario SET email = '%s', telefone = '%s', nome = '%s' WHERE id_usuario = '%s'"
    ...
    def locar_armario(self, nome, email, telefone, dia, hora, minuto, armario, language, total):
        ...
        if 'autorizada' in (resultado_transacao['PWINFO_RESULTMSG'].lower()).split():
            ...
            self.__c.execute(
                "INSERT INTO tb_locacao(id_locacao, data_locacao,tempo_locado,tempo_corrido,senha,id_armario,id_usuario) VALUES('%s', '%s','%s',null,'%s','%s','%s','%s')"
                % (
                    lista,
                    self.__data_locacao,
                    self.__data_limite,
            ...
            self.__c.execute(
                "INSERT INTO tb_locacao_persistence(id_locacao_persistence, data_locacao,tempo_locado,tempo_corrido,id_armario,id_usuario, valor_locacao) VALUES('%s', '%s','%s',null,'%s','%s','%s','%s', '%s')"
                % (
                    lista_persist,
                    self.__data_locacao,
                    self.__data_limite,
            ...
            self.__c.execute(
                "INSERT INTO tb_locacao_persistence(id_locacao_persistence, data_locacao,tempo_locado,tempo_corrido,id_armario,id_usuario, valor_locacao) VALUES('%s', '%s','%s',null,'%s','%s','%s', '%s')"
                % (
                    lista_persist,
                    self.__data_locacao,
                    self.__data_limite,
    ...
    def localisa_armario(self, classe):
        ...
        self.__c.execute(
            "SELECT id_armario  from tb_armario where classe = '%s' and ESTADO = 'LIVRE' ORDER BY id_armario"
            % (self.__classe,)
        )
    ...
    def select_user(senha):
        data_dao = DAO()
        __conn = mdb.connect(
            host=data_dao.db_host(),
            user=data_dao.db_user(),
            password=data_dao.db_passwd(),
            database=data_dao.db_database(),
        )
