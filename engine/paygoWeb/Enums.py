...
class E_PWOPER(enum.Enum):
    ...
    PWOPER_SETTLEMNT = (
        40  # (Fechamento/finalização) Finaliza a operação junto ao Provedor
    )
...
class E_PWCNF(enum.Enum):
    ...
    PWCNF_CNF_MANU_AUT = (
        12833  # A transação foi confirmada manualmente na Automação.*/
    )
    PWCNF_REV_MANU_AUT = (
        12849  # A transação foi desfeita manualmente na Automação.*/
    )
...
class E_PWINFO(enum.Enum):
    ...
    PWINFO_PRODUCTNAME = (
        42  # Nome/tipo do produto utilizado na nomenclatura do Provedor.
    )
    PWINFO_DATETIME = (
        49  # Data e hora local da transação no formato “AAAAMMDDhhmmss”
    )
    PWINFO_REQNUM = 50       # Referência local da transação
    PWINFO_AUTHSYST = (
        53  # Nome do Provedor: “ELAVON”; “FILLIP”; “LIBERCARD”; “RV”; etc
    )
    ...
    PWINFO_AUTLOCREF = (
        68  # Referência da transação para a infraestrutura Pay&Go Web
    )
    PWINFO_AUTEXTREF = (
        69  # Referência da transação para o Provedor (NSU host).
    )
    PWINFO_AUTHCODE = 70     # Código de autorização
    PWINFO_AUTRESPCODE = (
        71  # Código de resposta da transação (campo ISO8583:39)
    )
    PWINFO_AUTDATETIME = (
        72  # Data/hora da transação para o Provedor formato “AAAAMMDDhhmmss”.
    )
    ...
    PWINFO_CARDNAMESTD = (
        196  # Descrição do produto bandeira padrão relacionado ao BIN.
    )
    ...
    PWINFO_MERCHADDDATA1 = (
        240  # Dados adicionais relevantes para a Automação (#1)
    )
    PWINFO_MERCHADDDATA2 = (
        241  # Dados adicionais relevantes para a Automação (#2)
    )
    PWINFO_MERCHADDDATA3 = (
        242  # Dados adicionais relevantes para a Automação (#3)
    )
    PWINFO_MERCHADDDATA4 = (
        243  # Dados adicionais relevantes para a Automação (#4)
    )
    ...
    PWINFO_AUTHMNGTUSER = (
        245  # Identificador do usuário autenticado com a senha do lojista
    )
    PWINFO_AUTHTECHUSER = (
        246  # Identificador do usuário autenticado com a senha técnica.
    )
    PWINFO_PAYMNTTYPE = (
        7969  # Modalidade de pagamento: 1: cartão 2: dinheiro 3: cheque
    )
    ...
    PWINFO_PNDAUTHSYST = (
        32517  # Nome do provedor para o qual existe uma transação pendente.
    )
    PWINFO_PNDVIRTMERCH = 32518   # Identificador do Estabelecimento para o qual existe uma transação pendente
    PWINFO_PNDREQNUM = (
        32519  # Referência local da transação que está pendente.
    )
    PWINFO_PNDAUTLOCREF = 32520   # Referência para a infraestrutura Pay&Go Web da transação que está pendente.
    PWINFO_PNDAUTEXTREF = (
        32521  # Referência para o Provedor da transação que está pendente
    )
    PWINFO_LOCALINFO1 = (
        32522  # Texto exibido para um item de menu selecionado pelo usuário
    )
...
class E_PWRET(enum.Enum):
    ...
    PWRET_PPCOMERR = (
        -2484
    )              # Falha na comunicação com o PIN-pad (protocolo).
    PWRET_NOMANDATORY = (
        -2483
    ...
    PWRET_PPS_XXX = (
        -2200
    )               # Erros retornados pelo PIN-pad conforme seção 10.2
