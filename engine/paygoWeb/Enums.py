...
class E_PWOPER(enum.Enum):
    ...
    PWOPER_SALEVOID = (
        34  # (Cancelamento de venda) Cancela uma transação PWOPER_SALE realizando a transferência de fundos inversa
    )
    ...
    PWOPER_RETBALINQ = (
        37  # Consulta o saldo/limite do Estabelecimento (tipicamente limite de crédito para venda de créditos pré-pagos).
    )
    ...
    PWOPER_VOID = (
        57  # Exibe um menu com os cancelamentos disponíveis caso só exista um tipo este é selecionado automaticamente
    )
...
class E_PWCNF(enum.Enum):
    ...
    PWCNF_REV_DISP_AUT = (
        143665  # A transação foi desfeita pela Automação devido a uma falha no mecanismo de liberação da mercadoria.*/
    )
...
class E_PWINFO(enum.Enum):
    PWINFO_OPERATION = (
        2  # Tipo de transação (PWOPER_xxx). Consultar os valores possíveis na descrição da função PW_iNewTransac
    )
    ...
    PWINFO_DISCOUNTAMT = (
        73  # Valor do desconto concedido pelo Provedor considerando PWINFO_CURREXP já deduzido em PWINFO_TOTAMNT
    )
    ...
    PWINFO_INSTALLM1AMT = (
        79  # Valor da entrada para um pagamento parcelado considerando PWINFO_CURREXP já incluído em PWINFO_TOTAMNT
    )
    PWINFO_INSTALLMAMNT = 80   # Valor da parcela considerando PWINFO_CURREXP já incluído em PWINFO_TOTAMNT
    PWINFO_RCPTFULL = (
        82  # Comprovante para impressão – Via completa. Até 40 colunas quebras de linha identificadas pelo caractere 0Dh
    )
    ...
    PWINFO_TRNORIGDATE = (
        87  # Data da transação original no caso de um cancelamento ou uma confirmação de pré-autorização (formato “DDMMAA”).
    )
    ...
    PWINFO_TRNORIGAUTH = (
        98  # Código de autorização da transação original no caso de um cancelamento ou uma confirmação de pré-autorização
    )
    ...
    PWINFO_TRNORIGREQNUM = (
        114  # Número da solicitação da transação original no caso de um cancelamento ou uma confirmação de pré-autorização
    )
    PWINFO_TRNORIGTIME = (
        115  # Hora da transação original no caso de um cancelamento ou uma confirmação de pré-autorização (formato “HHMMSS”).
    )
    ...
    PWINFO_CARDFULLPAN = (
        193  # Número do cartão completo para transação digitada. Este dado não pode ser recuperado pela função PW_iGetResult
    )
    ...
    PWINFO_BARCODENTMODE = (
        233  # Modo(s) de entrada do código de barras: 1:  digitado; 2:  lido através de dispositivo eletrônico.
    )
    ...
    PWINFO_USINGPINPAD = (
        32513  # Indica se o ponto de captura faz ou não o uso de PIN-pad: 0: Não utiliza PIN-pad; 1: Utiliza PIN-pad.
    )
    ...
    PWINFO_IDLEPROCTIME = (
        32516  # Próxima data e horário em que a função PW_iIdleProc deve ser chamada pela Automação. Formato “AAMMDDHHMMSS”
    )
