...
class PGWebLibrary:
    ...
    def PW_iExecTransac(self, vstParam, iNumParam):
        ...
        self.PW_iExecTransacObj.argtypes = [
            POINTER((PW_GetData * 11)), POINTER(c_int)]
    ...
    def PW_iConfirmation(self, ulResult, pszReqNum, pszLocRef, pszExtRef, pszVirtMerch, pszAuthSyst):
        ...
        self.PW_iConfirmationObj.argtypes = [
            c_uint32, c_char_p, c_char_p, c_char_p, c_char_p, c_char_p]
    ...
    def PW_iPPGetUserData(self, uiMessageId, bMinLen, bMaxLen,  iToutSec, pszData):
        ...
        self.PW_iPPGetUserDataObj.argtypes = [
            c_short, c_short, c_short, c_short, POINTER(c_char)]

        ret = self.PW_iPPGetUserDataObj(c_short(uiMessageId), c_short(
            bMinLen), c_short(bMaxLen), c_short(iToutSec), pszData)
    ...
    def PW_iTransactionInquiry(self, pszXmlRequest, pszXmlResponse, ulXmlResponseLen):
        ...
        ret = self.PW_iTransactionInquiryObj(
            c_byte(pszXmlRequest), c_byte(pszXmlResponse), c_byte(ulXmlResponseLen))
        return ret
    ...
    def PW_iGetOperations(self, vstParam, piNumOperations):
        ...
        self.PW_iGetOperationsObj.argtypes = [
            POINTER((PW_Operations * 11)), POINTER(c_short)]
    ...
    def PW_iPPGetPINBlock(self, bKeyID, pszWorkingKey,  bMaxLen, iToutSec, pszPrompt, pszData):
        ...
        self.PW_iPPGetPINBlockObj.argtypes = [
            c_short, c_char_p, c_short, c_short, c_char_p, POINTER(c_char)]
    ...
    def PW_iExecGetData(self, vstParam, iNumParam):
        ...
        ulEvent = c_int32(0)
        szPrompt = c_buffer(84)

        szDspMsg = create_string_buffer(100000)

        print("vstParam[0].wIdentificador", vstParam[0].wIdentificador)
        print("vstParam[1].wIdentificador", vstParam[1].wIdentificador)
        print("vstParam[2].wIdentificador", vstParam[2].wIdentificador)
        print("vstParam[3].wIdentificador", vstParam[3].wIdentificador)
            ...
            if vstParam[i].bTipoDeDado == E_PWDAT.PWDAT_TYPED.value:
                ...
                if vstParam[i].bNumOpcoesMenu == 1:
                    ...
                    ret = self.PW_iAddParam(
                        vstParam[i].wIdentificador, vstParam[i].vszValorMenu.value)
                    print("uma opcao")
                    return ret
                    ...
                    pszMsg = ("1 - %s\n   2 - %s" %
                              (vstParam[i].vszTextoMenu[1].value, vstParam[i].vszTextoMenu[0].value))

                    retwait = ''

                    from window_select_cartao import WindowSelectCartao
                    ret = WindowSelectCartao()

                    if ret == 1:
                        ret = self.PW_iAddParam(
                            vstParam[i].wIdentificador, b'01')
                        retparc = self.PW_iAddParam(59, "1")
                        return ret
                    elif retwait == 2:
                        ret = self.PW_iAddParam(
                            vstParam[i].wIdentificador, b'02')
                        retparc = self.PW_iAddParam(59, "1")
                        return ret
