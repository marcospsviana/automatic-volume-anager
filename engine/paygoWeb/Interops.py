...
class PGWebLibrary:
    ...
    def PW_iConfirmation(
        ...
        return self.PW_iConfirmationObj(
            c_uint32(ulResult),
            c_char_p(pszReqNumAux),
            c_char_p(pszLocRefAux),
            c_char_p(pszExtRefAux),
            c_char_p(pszVirtMerchAux),
            c_char_p(pszAuthSystAux),
        )
    ...
    def PW_iTransactionInquiry(self, pszXmlRequest, pszXmlResponse, ulXmlResponseLen):
        ...
        ret = self.PW_iTransactionInquiryObj(
            c_byte(pszXmlRequest),
            c_byte(pszXmlResponse),
            c_byte(ulXmlResponseLen),
        )
        return ret
    ...
    def PW_iPPGetPINBlock(self, bKeyID, pszWorkingKey, bMaxLen, iToutSec, pszPrompt, pszData):
        ...
        return self.PW_iPPGetPINBlockObj(
            c_short(bKeyID),
            c_char_p(pszWorkingKey),
            c_short(bMaxLen),
            c_short(iToutSec),
            c_char_p(pszPrompt),
            pszData,
